...
async def get_all_sticker_packs(c: QuantamBot, user_id: int, offset: int = 1, limit: int = 25):
    packnum = 25 * (offset - 1)
...
async def resize_file_to_sticker_size(file_path: str, length: int = 512, width: int = 512) -> str:
    ...
    file_pathh = f"r{str(time()).replace('.', '_')}.png"
    im.save(file_pathh)
    os.remove(file_path)
    return file_pathh
...
async def Vsticker(c: QuantamBot, file: Message):
    if file.animation:
        file = file.animation
    elif file.video:
        file = file.video
...
async def upload_document(
        client: QuantamBot, file_path: str, chat_id: int
...
async def remove_sticker(c: QuantamBot, stickerid: str) -> raw.base.messages.StickerSet:
...
async def draw_meme(image_path: str, text: str, sticker: bool, fiill: str) -> list:
    ...
    font = ImageFont.truetype("./Merisa/utils/font.otf", font_size)
    ...
    image1 = image_path
    image2 = tosticker(stick_path, f"@Quantambot_{int(time())}.webp")
