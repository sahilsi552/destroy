...
from..utils.fonts_ import Fonts
@QuantamBot.on_message(filters.command(["font", "fonts"]))
async def style_buttons(c, m, cb=False):
    ...
    buttons = [
        ...
        [InlineKeyboardButton ("ᴄʟᴏsᴇ",callback_data="closeforce"),InlineKeyboardButton ("ɴᴇxᴛ ➻", callback_data="nxt")],
    ]
    if not cb:
        await m.reply_text(
...
@QuantamBot.on_callback_query(filters.regex("^nxt"))
async def nxt(c, m:CallbackQuery):
    if m.data == "nxt":
        buttons = [
            ...
            [InlineKeyboardButton ("ᴄʟᴏsᴇ",callback_data="closeforce")]
        ]
        await m.answer()
        await m.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
    else:
...
@QuantamBot.on_callback_query(filters.regex("^style"))
async def style(c, m:CallbackQuery):
    ...
    try:
        await m.message.edit_text(f"ʏᴏᴜʀ ꜱᴛʏʟᴇ ꜰᴏɴᴛ\n`{new_text}`")
