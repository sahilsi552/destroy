...
PRVT_MSGS = {}
@QuantamBot.on_inline_query()
async def inline_menu(c: QuantamBot, inline_query: InlineQuery):
    if inline_query.query.strip().lower().strip() == "":
        ...
        try:
            penerima = await QuantamBot.get_users(_id.strip())
...
__help__=f"""ᴛᴏ ᴜsᴇ ᴛʜɪs ғᴇᴀᴛᴜʀᴇ, ᴊᴜsᴛ ᴛʏᴘᴇ ʙᴏᴛ ᴜsᴇʀɴᴀᴍᴇ [@{QuantamBot.username}] ᴡɪᴛʜ ғᴏʟʟᴏᴡɪɴɢ ᴀʀɢs ʙᴇʟᴏᴡ.
