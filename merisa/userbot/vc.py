...
async def startvc(client: Client, message: Message):
    ...
    # c = await get_assistant(message.chat.id)
...
@Client.on_message(filters.command("endvc", prefixes=["."])& filters.me)
...
async def vcmembers(c, message: Message):
    ...
    try:
        full_chat: base.messages.ChatFull = await c.invoke(
            GetFullChannel(channel=(await c.resolve_peer(message.chat.id)))
        )
        participants: base.phone.GroupParticipants = await c.invoke(
            GetGroupParticipants(
        ...
        if "'NoneType' object has no attribute 'write'" in str(e):
            await hell.edit_text("vc is  off ")
        else:
            logging.exception(e)
